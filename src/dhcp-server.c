...
bool dhcp_server_consume(struct lan_play *lp, const uint8_t *frame, size_t len)
{
    ...
    if (req.msg_type == DHCP_MSG_REQUEST) {
        /* A REQUEST naming another server is not ours to answer. */
        if (req.has_server_id && !CMP_IPV4(req.server_id, lp->wifi_ip)) return true;
        ...
        if (lease && lease->expires > time(NULL) && has_want && CMP_IPV4(want, lease->ip)) {
